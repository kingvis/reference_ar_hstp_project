% Safety defaults, used only if you run this file on its own to test it:
%if isempty(Hs);        Hs        = 2.0;       end
%if isempty(Tp);        Tp        = 8.0;       end
...
waves.height = Hs;
waves.period= Tp;
waves.spectrumType = 'JS';  % PM-Pierson–Moskowitz JS-JONSWAP 
waves.gamma = 3.3;          % Common default JONSWAP peak factor
...
% Variant control flags (place in base workspace before opening model)
sv_b_dragBody  = 1;   % 1 = enable drag variant, 0 = disable
sv_b_hydroBody = 1;   % 1 = enable hydrodynamic variant, 0 = disable

% (Optional) Create Simulink.VariantExpression objects to reference by name
v_drag  = Simulink.VariantExpression('sv_b_dragBody==1');
v_hydro = Simulink.VariantExpression('sv_b_hydroBody==1');
